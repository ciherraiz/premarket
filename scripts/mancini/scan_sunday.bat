...
echo [%date% %time%] Arrancando scan Mancini domingo... >> logs\mancini_scheduler.log

uv run python scripts/mancini/run_mancini.py scan >> logs\mancini_scheduler.log 2>&1

echo [%date% %time%] Scan domingo finalizado (exit code: %ERRORLEVEL%) >> logs\mancini_scheduler.log
